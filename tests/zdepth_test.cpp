...
bool TestFrame(const uint16_t* frame, bool keyframe)
{
    ...
    compressor.Compress(Width, Height, frame, compressed, keyframe, true);

    const uint64_t t1 = GetTimeUsec();

    int width, height;
    std::vector<uint16_t> depth;
    zdepth::DepthResult result = decompressor.Decompress(true, compressed, width, height, depth);
    ...
    QuantizeDepthImage(true, Width, Height, frame, quantized);
    ...
    DequantizeDepthImage(true, Width, Height, quantized.data(), depth);
